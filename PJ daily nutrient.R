...
#GW <- read.csv("/home/ubuntu/WP3/PJ/data/GW.csv", header = T)
...
#names(GW) <- c("Date", "WaterLevel", "AHD")
...
## merge the three datasets for combined_TSS 
combined_nutrient_unscaled <- function(dataset, n_target) {
  ...
  ## add the GW infromtion into the dataset 
  #combined_target <- merge(GW_week, combined_target, by = 'week', all.y = T)
  combined_target$Date <- as.numeric(combined_target$Date)
  
  # Normalize the nutrient_target data
  #combined_target[is.na(combined_target$week_GW),"week_GW"]<--GW_mean
